...
function searchRecipe(event) {
  ...
  // Display "search not found" if no recipes match the search query
  const searchNotFoundMessage = document.getElementById('searchNotFoundMessage');
  if (visibleRecipes === 0 && searchQuery !== '') { // Check if searchQuery is not empty
...
function submitFeedback(event) {
  ...
  // Process the ratings and feedback (You can send this data to your server or store it locally)
  console.log("Rating: ", ratingValue);
  console.log("Feedback: ", feedbackText);

  // Optionally, you can display a thank you message or close the popup after submitting feedback
  // For example:
  alert("Thank you for your feedback!");
  closePopup();
}

// JavaScript for food category pop-up
function toggleFoodCategories() {
  var popup = document.getElementById("foodCategoryPopup");
  popup.classList.toggle("show");
}

function closeFoodCategories() {
  var popup = document.getElementById("foodCategoryPopup");
